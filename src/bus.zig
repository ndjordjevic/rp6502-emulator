...
pub fn readByte(addr: u16, is_dbg: bool) callconv(.c) u8 {
    ...
    // RIA region ($FFE0–$FFFF): placeholder reads from RAM for vectors ($FFFA–$FFFF)
    // until RIA registers are implemented. This allows tests to set reset vectors.
    return memory.ram[addr];
}
...
pub fn writeByte(addr: u16, val: u8) callconv(.c) void {
    ...
    // $FF00–$FFCF: unassigned — ignore writes
    // $FFD0–$FFDF: VIA stub — ignore writes
    // $FFE0–$FFFF: RIA placeholder — write to RAM for vector setup until RIA is implemented
    if (addr >= RIA_START) {
        memory.ram[addr] = val;
    }
}
